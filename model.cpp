...
static std::string convert_diffusers_name_to_compvis(std::string key, char seq) {
    ...
    if (match(m, std::regex(sd_format("unet%cadd_embedding%clinear_(\\d+)(.*)", seq, seq)), key)) {
        return sd_format("model%cdiffusion_model%clabel_emb%c0%c", seq, seq, seq, seq) + std::to_string(std::stoi(m[0]) * 2 - 2) + m[1];
    }
    ...
    // clip-g
    if (match(m, std::regex(sd_format("te%c1%ctext_model%cencoder%clayers%c(\\d+)%c(.+)", seq, seq, seq, seq, seq, seq)), key)) {
        return sd_format("cond_stage_model%c1%ctransformer%ctext_model%cencoder%clayers%c", seq, seq, seq, seq, seq, seq) + m[0] + seq + m[1];
    }

    if (match(m, std::regex(sd_format("te%c1%ctext_model(.*)", seq, seq)), key)) {
        return sd_format("cond_stage_model%c1%ctransformer%ctext_model", seq, seq, seq) + m[0];
    }

    if (match(m, std::regex(sd_format("te%c1%ctext_projection", seq, seq)), key)) {
        return sd_format("cond_stage_model%c1%ctransformer%ctext_model%ctext_projection", seq, seq, seq, seq);
    }
...
bool ModelLoader::init_from_safetensors_file(const std::string& file_path, const std::string& prefix, int n_threads_p) {
    ...
    auto process_chunk = [&](int thread_id, size_t begin, size_t end) {
...
#ifdef __EMSCRIPTEN__
    // Emscripten without pthreads cannot construct std::thread — run inline
    for (int thread_id = 0; thread_id < n_threads; ++thread_id) {
        size_t begin = static_cast<size_t>(thread_id) * chunk_size;
        if (begin >= read_order.size()) { continue; }
        size_t end = std::min(begin + chunk_size, read_order.size());
        process_chunk(thread_id, begin, end);
    }
#else
...
bool ModelLoader::load_tensors(on_new_tensor_cb_t on_new_tensor_cb, int n_threads_p, bool enable_mmap) {
    ...
    for (size_t file_index = 0; file_index < file_paths_.size(); file_index++) {
        ...
        auto worker_fn = [&](const std::string& file_path_arg, bool is_zip_arg) {
            std::ifstream file;
            struct zip_t* zip = nullptr;
            if (is_zip_arg) {
                zip = zip_open(file_path_arg.c_str(), 0, 'r');
                if (zip == nullptr) {
                    LOG_ERROR("failed to open zip '%s'", file_path_arg.c_str());
                    failed = true;
                    return;
                }
            } else if (!mmapped) {
                file.open(file_path_arg, std::ios::binary);
                if (!file.is_open()) {
                    LOG_ERROR("failed to open '%s'", file_path_arg.c_str());
                    failed = true;
                    return;
                }
            }
                ...
                auto read_data = [&](char* buf, size_t n) {
                    if (zip != nullptr) {
                        ...
                        if (!mmapped->copy_data(buf, n, tensor_storage.offset)) {
                            LOG_ERROR("read tensor data failed: '%s'", file_path_arg.c_str());
                            failed = true;
                        }
                    } else {
        ...
        for (int i = 0; i < n_threads; ++i) {
            worker_fn(file_path, is_zip);
        }
#else
        std::vector<std::thread> workers;
        for (int i = 0; i < n_threads; ++i) {
            workers.emplace_back(worker_fn, file_path, is_zip);
        }
