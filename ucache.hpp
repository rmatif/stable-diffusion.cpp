...
struct UCacheState {
    ...
    float prev_output_norm                = 0.0f;
    bool has_prev_input                   = false;
    bool has_prev_output                  = false;
    bool has_prev_output_norm             = false;
    ...
    float prev_c_in                       = 1.0f;
    bool has_prev_c_in                    = false;
    ...
    void reset_runtime() {
        ...
        prev_input.clear();
        prev_output.clear();
        prev_output_norm                = 0.0f;
        has_prev_input                  = false;
        has_prev_output                 = false;
        has_prev_output_norm            = false;
    ...
    float current_c_in(float sigma) const {
        if (!denoiser) {
            return 1.0f;
        }
    ...
    void update_cache(const SDCondition* cond, ggml_tensor* input, ggml_tensor* output, float sigma) {
        ...
        entry.cached_c_in  = current_c_in(sigma);
        entry.cached_sigma = sigma;
    }

    void apply_cache(const SDCondition* cond, ggml_tensor* input, ggml_tensor* output, float sigma) {
        auto it = cache_diffs.find(cond);
        ...
        float scale = 1.0f;
        float c_in  = current_c_in(sigma);
        if (it->second.cached_c_in > 0.0f) {
            scale *= c_in / it->second.cached_c_in;
        }
        if (it->second.cached_sigma > 0.0f) {
            float sigma_ratio = sigma / it->second.cached_sigma;
            if (sigma_ratio > 4.0f) {
                sigma_ratio = 4.0f;
            } else if (sigma_ratio < 0.25f) {
                sigma_ratio = 0.25f;
            }
            scale *= std::sqrt(sigma_ratio);
        }
        if (scale > 2.0f) {
            scale = 2.0f;
        } else if (scale < 0.1f) {
    ...
    bool before_condition(const SDCondition* cond,
        ...
        if (skip_current_step) {
            if (has_cache(cond)) {
                apply_cache(cond, input, output, sigma);
                return true;
            }
            return false;
        }
        ...
        if (has_prev_output_norm && has_relative_transformation_rate && last_input_change > 0.0f && prev_output_norm > 0.0f) {
            float sigma_gain = 1.0f;
            if (has_last_sigma && last_sigma > 0.0f) {
                float ratio = sigma / last_sigma;
                if (ratio > 4.0f) {
                    ratio = 4.0f;
                } else if (ratio < 0.25f) {
                    ratio = 0.25f;
                }
                sigma_gain = std::sqrt(ratio);
            }

            float c_in_gain = 1.0f;
            float c_in      = current_c_in(sigma);
            if (has_prev_c_in && prev_c_in > 0.0f) {
                c_in_gain = c_in / prev_c_in;
                if (c_in_gain > 2.0f) {
                    c_in_gain = 2.0f;
                } else if (c_in_gain < 0.25f) {
                    c_in_gain = 0.25f;
                }
            }

            float approx_output_change_rate = (relative_transformation_rate * last_input_change * (0.6f * sigma_gain + 0.4f * c_in_gain)) / prev_output_norm;
    ...
    void after_condition(const SDCondition* cond, ggml_tensor* input, ggml_tensor* output, float sigma) {
        if (!step_is_active()) {
            return;
        }
        update_cache(cond, input, output, sigma);
        ...
        prev_output_norm     = (ne > 0) ? (mean_abs / static_cast<float>(ne)) : 0.0f;
        has_prev_output_norm = prev_output_norm > 0.0f;
